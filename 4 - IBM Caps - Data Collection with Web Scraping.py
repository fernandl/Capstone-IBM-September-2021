...
soup_f9 = BeautifulSoup(html_data,"html.parser")
soup_f9
...
html_tables = soup_f9.find_all('table')
html_tables
...
columns_first_launch = first_launch_table.find_all('th')
columns_first_launch
...
for row in columns_first_launch:
...
extracted_row = 0
#Extract each table 
for table_number,table in enumerate(first_launch_table.find_all('table',"wikitable plainrowheaders collapsible")):
   # get table row 
...
# df=pd.DataFrame(launch_dict) - this one is not working == "arrays must all be same length"
